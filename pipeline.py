...
cars = []
notcars = []
vehicles_folders = glob.glob('vehicles/*/')
for folders in vehicles_folders:
    images = glob.glob(folders + '*.png')
    for image in images:
        cars.append(image)

non_vehicles_folders = glob.glob('nonvehicles/*/')
for folders in non_vehicles_folders:
    images = glob.glob(folders + '*.png')
    for image in images:
...
kwargs = {'color_space': 'YCrCb', 'spatial_size': (32, 32), 'hist_bins': 32,
          'cell_per_block': 2, 'hog_channel': 'ALL',
          'pix_per_cell': 8, 'orient': 9,
          'spatial_feat': True, 'hist_feat': True, 'hog_feat': True}
